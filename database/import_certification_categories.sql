...
(11, '化学检测', 'Chemical Testing', 1, 10) AS new
ON DUPLICATE KEY UPDATE 
    name_cn = new.name_cn,
    name_en = new.name_en,
    parent_id = new.parent_id,
...
(21, '德国', 'Germany', 2, 10),
(22, '西班牙', 'Spain', 2, 11),
(23, '英国', 'United Kingdom', 2, 12) AS new
ON DUPLICATE KEY UPDATE 
    name_cn = new.name_cn,
    name_en = new.name_en,
    parent_id = new.parent_id,
...
-- 三级分类：国际认证下的国家/地区
INSERT INTO certification_category (id, name_cn, name_en, parent_id, sort_order) VALUES
(39, '国际', 'International', 6, 1) AS new
ON DUPLICATE KEY UPDATE 
    name_cn = new.name_cn,
    name_en = new.name_en,
    parent_id = new.parent_id,
